#!/bin/sh
# flowguard-escalation-cfg.sh — GET lista a config de bloqueio progressivo por
# reincidência (scan); POST { changes: {chave: valor, ...} } aplica via socket.
...
try:
    ...
    if not isinstance(changes, dict) or not changes:
        ...
        resp = control.send_command(cfg["daemon"]["socket"], {"cmd": "escalation_cfg_set", "changes": changes})
...
try:
    cfg = yaml.safe_load(open("/root/flowguard/config.yaml", encoding="utf-8"))
    resp = control.send_command(cfg["daemon"]["socket"], {"cmd": "escalation_cfg"})
    print(json.dumps(resp))
except Exception as exc:
    print(json.dumps({"ok": False, "error": str(exc)}))
PYEOF
