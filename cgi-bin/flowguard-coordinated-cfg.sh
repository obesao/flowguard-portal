#!/bin/sh
# flowguard-coordinated-cfg.sh — GET lista a config de detecção de destino
# coordenado (N src externos -> 1 host/porta protegido); POST { changes: {...} }
# aplica mudanças via socket do daemon.
...
try:
    ...
    if not isinstance(changes, dict) or not changes:
        ...
        resp = control.send_command(cfg["daemon"]["socket"],
                                     {"cmd": "coordinated_destination_cfg_set", "changes": changes})
...
try:
    cfg = yaml.safe_load(open("/root/flowguard/config.yaml", encoding="utf-8"))
    resp = control.send_command(cfg["daemon"]["socket"], {"cmd": "coordinated_destination_cfg"})
    print(json.dumps(resp))
except Exception as exc:
    print(json.dumps({"ok": False, "error": str(exc)}))
PYEOF
